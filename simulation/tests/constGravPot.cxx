...
int main(int argc, char *argv[]){
	...
	initialize_wavefunction(Ng, CC, TC, params.rL(), params.zin(), params.iseed(), d, wf);
	...
	MPI_Barrier(MPI_COMM_WORLD);
	solver->forward_solve(wf.m_phi.data());
	MPI_Barrier(MPI_COMM_WORLD);
	
	writePk(solver, params, params.outBase(), "phi.ini");

	MPI_Barrier(MPI_COMM_WORLD);
	writeVector(wf.m_phi, wf, "phi.ini"); 
	MPI_Barrier(MPI_COMM_WORLD);
